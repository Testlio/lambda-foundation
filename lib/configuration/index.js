...
//  and is assumed to be whatever the CWD was during bundling.
//
module.exports = function(configurationDirectory) {
    ...
    const defaultConfig = loadDefaultConfig(configurationDirectory);

    // Load env specific conf
    const environmentConfig = loadEnvironmentConfig(process.env.NODE_ENV || 'development', configurationDirectory);

    // Check if there are env vars that need to be mapped
    const envVarsConfig = loadEnvironmentVariablesConfig(configurationDirectory);
